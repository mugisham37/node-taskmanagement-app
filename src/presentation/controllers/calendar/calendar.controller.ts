...
export const validateCreateCalendarEvent = validate(
  calendarValidators.createCalendarEvent
);
export const validateUpdateCalendarEvent = validate(
  calendarValidators.updateCalendarEvent
);
export const validateGetCalendarEvent = validate(
  calendarValidators.getCalendarEvent
);
export const validateDeleteCalendarEvent = validate(
  calendarValidators.deleteCalendarEvent
);
export const validateGetCalendarEvents = validate(
  calendarValidators.getCalendarEvents
);
export const validateGetCalendarEventStats = validate(
  calendarValidators.getCalendarEventStats
);
export const validateProcessEventReminders = validate(
  calendarValidators.processEventReminders
);
 ...
 * @access  Private
 */
export const getCalendarEventById = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const userId = req.user?.id as string;
    const eventId = req.params.id;
    const calendarEvent = await calendarEventService.getCalendarEventById(
      eventId,
      {
        userId,
        timestamp: new Date(),
 ...
 * @access  Private
 */
export const getCalendarEventStats = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const userId = req.user?.id as string;
    ...
    const stats = await calendarEventService.getCalendarEventStats(
      userId,
      dateRange,
      {
        userId,
    ...
    successResponse(
      ...
      200,
      stats,
      'Calendar event statistics retrieved successfully'
    );
  }
);
 ...
 */
export const processEventReminders = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const userId = req.user?.id as string;
    ...
    successResponse(
      ...
      200,
      { remindersSent },
      'Event reminders processed successfully'
    );
  }
);
