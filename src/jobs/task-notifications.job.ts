...
export function scheduleTaskNotificationsJob(interval: number): NodeJS.Timeout {
  logger.info(
    `Scheduling task notifications job to run every ${interval / 1000} seconds`,
    {
...
async function processTaskNotifications(): Promise<void> {
  ...
  try {
    ...
    logger.info('Task notifications processing completed successfully', {
      ...
      totalNotifications:
        overdueResults.notificationsSent + upcomingResults.notificationsSent,
 ...
 */
async function processOverdueTasks(): Promise<{
 ...
 * Process upcoming due date reminders
 */
async function processUpcomingDueDateReminders(): Promise<{
  notificationsSent: number;
  errors: string[];
}> {
  const result = { notificationsSent: 0, errors: [] as string[] };
  ...
  try {
    ...
    for (const task of upcomingTasks.data) {
      try {
        ...
        logger.error(
          `Failed to send upcoming due date reminder for task ${task.id}`,
          {
            taskId: task.id,
            error: errorMessage,
          }
...
export async function checkOverdueTasksJob(): Promise<void> {
  try {
    ...
    logger.info(
      `Legacy job completed: Sent ${result.notificationsSent} overdue task notifications`,
      {
        jobType: 'overdue-tasks-legacy',
...
export function scheduleOverdueTasksJob(interval = 3600000): NodeJS.Timeout {
  logger.info(
    `Scheduling overdue tasks job to run every ${interval / 1000} seconds`,
    {
      jobType: 'overdue-tasks-legacy',
      intervalMs: interval,
      intervalSeconds: interval / 1000,
