...
/**
 * Safe SQL condition builders to handle null/undefined values
 */
...
export class APIFeatures<T extends PgTable> {
  ...
  filter(): APIFeatures<T> {
    ...
    if (this.queryString.fromDate || this.queryString.toDate) {
      const dateColumn =
        this.getColumnByName('dueDate') || this.getColumnByName('createdAt');
      if (dateColumn) {
        try {
          if (this.queryString.fromDate && this.queryString.toDate) {
            conditions.push(
              between(
                dateColumn,
                new Date(this.queryString.fromDate),
                new Date(this.queryString.toDate)
              )
            );
          } else if (this.queryString.fromDate) {
            conditions.push(
              gte(dateColumn, new Date(this.queryString.fromDate))
            );
  ...
  async execute(): Promise<PaginationResult<any>> {
    ...
    const [data, totalResult] = await Promise.all([this.query, countQuery]);
  ...
  private getColumnByName(columnName: string): PgColumn | undefined {
    try {
      ...
      const columns =
        (this.table as any)[Symbol.for('drizzle:Columns')] ||
        (this.table as any)._.columns;
