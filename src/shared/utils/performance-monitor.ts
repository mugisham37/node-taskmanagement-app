...
export const getPerformanceStats = (): {
  ...
  const slowRequests = performanceMetrics.filter(
    m =>
      m.duration > SLOW_REQUEST_THRESHOLD &&
      m.duration <= VERY_SLOW_REQUEST_THRESHOLD
  ).length;

  const verySlowRequests = performanceMetrics.filter(
    m => m.duration > VERY_SLOW_REQUEST_THRESHOLD
  ).length;

  const errorRequests = performanceMetrics.filter(
    m => m.statusCode >= 400
  ).length;
  const errorRate =
 ...
 */
export const getMetricsByEndpoint = (): Record<
  string,
  {
    count: number;
    averageResponseTime: number;
