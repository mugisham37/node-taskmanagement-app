# Multi-stage Dockerfile for Admin application
...
COPY --from=build --chown=nodejs:nodejs /app/apps/admin/.next ./apps/admin/.next
COPY --from=build --chown=nodejs:nodejs /app/apps/admin/public ./apps/admin/public
COPY --from=build --chown=nodejs:nodejs /app/apps/admin/package.json ./apps/admin/
COPY --from=build --chown=nodejs:nodejs /app/apps/admin/next.config.js ./apps/admin/
...
LABEL version="1.0.0"
LABEL description="TaskManagement Admin Dashboard"
