...
export function registerMigrationServices(container: Container): void {
  ...
  container.registerSingleton(MIGRATION_SERVICE_TOKENS.MIGRATION_TRACKER, MigrationTrackerService);
  container.registerSingleton(MIGRATION_SERVICE_TOKENS.BACKUP_SERVICE, BackupService);
  container.registerSingleton(MIGRATION_SERVICE_TOKENS.FILE_ANALYSIS, FileAnalysisService);
  container.registerSingleton(MIGRATION_SERVICE_TOKENS.SYSTEM_MAPPER, CurrentSystemMapperService);
  container.registerSingleton(MIGRATION_SERVICE_TOKENS.VERIFICATION_SERVICE, VerificationService);

  // Register error recovery service with dependencies
  container.registerSingleton(MIGRATION_SERVICE_TOKENS.ERROR_RECOVERY, ErrorRecoveryService, [
