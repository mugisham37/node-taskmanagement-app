...
export class ComprehensiveSecurityMiddleware {
   ...
   */
  securityHeaders() {
    return async (_req: FastifyRequest, reply: FastifyReply, next?: () => void) => {
   ...
   */
  ipFilter() {
    return async (req: FastifyRequest, _reply: FastifyReply, next?: () => void) => {
  ...
  userAgentFilter() {
    return async (req: FastifyRequest, _reply: FastifyReply, next?: () => void) => {
      ...
      if (this.config.userAgentBlacklist) {
        const isBlocked = this.config.userAgentBlacklist.some((pattern) => pattern.test(userAgent));
