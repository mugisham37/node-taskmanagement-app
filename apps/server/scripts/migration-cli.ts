...
import {
  registerMigrationServices,
  MIGRATION_SERVICE_TOKENS,
} from '../src/infrastructure/migration/migration-service-registration';
import { MigrationTrackerService } from '../src/infrastructure/migration/services/migration-tracker.service';
import { FileAnalysisService } from '../src/infrastructure/migration/services/file-analysis.service';
import { CurrentSystemMapperService } from '../src/infrastructure/migration/services/current-system-mapper.service';
import { BackupService } from '../src/infrastructure/migration/services/backup.service';
...
class MigrationCLI {
  ...
  async initialize() {
    ...
    registerMigrationServices(this.container);

    // Resolve services
    this.migrationTracker = this.container.resolve<MigrationTrackerService>(
      MIGRATION_SERVICE_TOKENS.MIGRATION_TRACKER
    );
    this.fileAnalysis = this.container.resolve<FileAnalysisService>(
      MIGRATION_SERVICE_TOKENS.FILE_ANALYSIS
    );
    this.systemMapper = this.container.resolve<CurrentSystemMapperService>(
      MIGRATION_SERVICE_TOKENS.SYSTEM_MAPPER
    );
    this.backupService = this.container.resolve<BackupService>(
