...
export class CacheWarmer {
  ...
  private async warmupUserProfile(userId: string): Promise<void> {
    ...
    await this.cacheService.set(`user:${userId}`, userProfile, {
      ttl: 1800, // 30 minutes
    });
  }
  ...
  private async warmupUserProjects(userId: string): Promise<void> {
    ...
    await this.cacheService.set(`user:${userId}:projects`, userProjects, {
      ttl: 1800,
    });
  }
  ...
  private async warmupUserTasks(userId: string): Promise<void> {
    ...
    await this.cacheService.set(`user:${userId}:tasks`, userTasks, {
      ttl: 900, // 15 minutes
    });
  }
  ...
  private async warmupProjectDetails(projectId: string): Promise<void> {
    ...
    await this.cacheService.set(`project:${projectId}`, projectDetails, {
      ttl: 1800,
    });
  }
  ...
  private async warmupProjectMembers(projectId: string): Promise<void> {
    ...
    await this.cacheService.set(`project:${projectId}:members`, projectMembers, {
      ttl: 1800,
    });
  }
  ...
  private async warmupProjectTasks(projectId: string): Promise<void> {
    ...
    await this.cacheService.set(`project:${projectId}:tasks`, projectTasks, {
      ttl: 900,
    });
  }
  ...
  private async warmupProjectStats(projectId: string): Promise<void> {
    ...
    await this.cacheService.set(`project:${projectId}:stats`, projectStats, {
      ttl: 3600,
    });
  }
...
export function createCacheWarmer(
  ...
  const defaultConfig: WarmupConfig = {
    enabled: true,
    strategies: ['config', 'user-data', 'project-data', 'stats'],
    batchSize: 10,
    delayBetweenBatches: 100, // 100ms
    ...config,
  };

  return new CacheWarmer(cacheService, defaultConfig);
}