...
import { PasswordService, PasswordStrength } from './interfaces';
...
export class DefaultPasswordService implements PasswordService {
  readonly name = 'password-service';
  ...
  async hash(password: string): Promise<string> {
    try {
      ...
      if (!strength.isValid) {
        throw new Error(`Password validation failed: ${strength.feedback.join(', ')}`);
      }
  ...
  async isHealthy(): Promise<boolean> {
    try {
      ...
      const isValid = await this.verify(testPassword, hash);
      return isValid;
    } catch (error) {
      return false;
    }
  ...
  async getHealthStatus(): Promise<Record<string, any>> {
    ...
    return {
      ...
      requirements: {
        uppercase: this.config.requireUppercase,
        lowercase: this.config.requireLowercase,
        numbers: this.config.requireNumbers,
        specialChars: this.config.requireSpecialChars,
      },
    };
  }
  ...
  private validateConfig(): void {
    if (this.config.minLength < 1) {
      throw new Error('Minimum password length must be at least 1');
    }

    if (this.config.maxLength < this.config.minLength) {
      throw new Error('Maximum password length must be greater than minimum length');
    }

    if (this.config.hashingOptions.memoryCost < 1024) {
      throw new Error('Argon2 memory cost must be at least 1024 KB');
    }

    if (this.config.hashingOptions.timeCost < 1) {
      throw new Error('Argon2 time cost must be at least 1');
    }

    if (this.config.hashingOptions.parallelism < 1) {
      throw new Error('Argon2 parallelism must be at least 1');
    }

    if (this.config.hashingOptions.hashLength < 16) {
      throw new Error('Argon2 hash length must be at least 16 bytes');
    }
  }
}