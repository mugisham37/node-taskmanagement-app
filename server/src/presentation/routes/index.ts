  ...
  try {
    ...
    await app.register(async fastify => {
      ...
      await fastify.register(async fastify => {
        const authController = container.resolve(SERVICE_TOKENS.AUTH_CONTROLLER);
        await authRoutes(fastify, authController as any, authMiddleware as any, rateLimitMiddleware as any);
      }, { prefix: '/auth' });

      // Task routes
      await fastify.register(async fastify => {
        const taskController = container.resolve(SERVICE_TOKENS.TASK_CONTROLLER);
        await taskRoutes(fastify, taskController as any, authMiddleware as any, rateLimitMiddleware as any);
      }, { prefix: '/tasks' });

      // Project routes
      await fastify.register(async fastify => {
        const projectController = container.resolve(SERVICE_TOKENS.PROJECT_CONTROLLER);
        const taskController = container.resolve(SERVICE_TOKENS.TASK_CONTROLLER);
        await projectRoutes(fastify, projectController as any, taskController as any, authMiddleware as any, rateLimitMiddleware as any);
      }, { prefix: '/projects' });

      // Workspace routes
      await fastify.register(async fastify => {
        const workspaceController = container.resolve(SERVICE_TOKENS.WORKSPACE_CONTROLLER);
        const projectController = container.resolve(SERVICE_TOKENS.PROJECT_CONTROLLER);
        await workspaceRoutes(fastify, workspaceController as any, projectController as any, authMiddleware as any, rateLimitMiddleware as any);
      }, { prefix: '/workspaces' });

      // User routes
      await fastify.register(async fastify => {
        const userController = container.resolve(SERVICE_TOKENS.USER_CONTROLLER);
        await userRoutes(fastify, userController as any, authMiddleware as any, rateLimitMiddleware as any);
      }, { prefix: '/users' });
